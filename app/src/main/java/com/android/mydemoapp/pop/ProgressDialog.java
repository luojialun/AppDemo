package com.android.mydemoapp.pop;
...
/**
 * Created by luojialun on 2018/12/10.
 */
public class ProgressDialog extends Dialog {
     ...
     * @return
     */
    public synchronized static ProgressDialog showDialog(Context context, CharSequence message, boolean cancelable, OnCancelListener cancelListener) {
        dialog = new ProgressDialog(context, R.style.Custom_Progress);
        dialog.setTitle("");
        dialog.setContentView(R.layout.progress_dialog);
        if (message == null || message.length() == 0) {
            dialog.findViewById(R.id.message).setVisibility(View.GONE);
        } else {
            txt = dialog.findViewById(R.id.message);
            txt.setText(message);
        }
        // 设置点击屏幕Dialog不消失
