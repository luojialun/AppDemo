...
public class SystemBarHelper {
    ...
    public static void immersiveStatusBar(Activity activity, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M||isFlyme4Later()||isMIUI6Later()){
            immersiveStatusBar(activity.getWindow(), alpha);
        }else {
            //6.0以下设置状态栏透明度为0.1f
            immersiveStatusBar(activity.getWindow(), 0.1f);
        }
    }
    ...
    /**
     * 将填充的状态栏高度设置为真正状态栏的高度
     * @param statusBar 填充的假的状态栏
     */
    public static void resizeStatusBarHeight(View statusBar){
        if (statusBar==null){
            return;
        }
        ViewGroup.LayoutParams statusLayoutParams = statusBar.getLayoutParams();
        statusLayoutParams.height = NotchScreenUtils.getStatusBarHeight();
        statusBar.setLayoutParams(statusLayoutParams);
    }
}